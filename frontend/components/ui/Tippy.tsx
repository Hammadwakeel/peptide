...
export function HintPopover({ hint, label = "Field help" }: HintPopoverProps) {
  return (
    <Popover content={<p className="text-sm leading-relaxed">{hint}</p>} trigger="mouseenter focus click">
      <button
        type="button"
        className="inline-flex size-5 items-center justify-center rounded-full border border-deep-teal/15 text-[11px] font-medium text-deep-teal/50 transition-colors hover:border-pacific-teal hover:text-pacific-teal"
        aria-label={label}
      >
        ?
      </button>
