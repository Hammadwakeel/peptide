...
def create_sub_affiliate(
    ...
    cursor.execute(
        """
        INSERT INTO affiliates (user_id, affiliate_code, affiliate_type, parent_affiliate_id, status)
        VALUES (%s, %s, 'sub', %s, 'active')
        RETURNING id, user_id, affiliate_code, affiliate_type::text AS affiliate_type,
                  parent_affiliate_id, status::text AS status
        """,
        (user_id, affiliate_code, parent_affiliate_id),
    )
    return _row_to_dict(cursor, cursor.fetchone())
